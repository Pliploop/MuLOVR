...
class DataModuleSplitter:
    ...
    def __init__(
        ...
        if self.task is None:
            ...
            fetch_function = eval(f"self.get_{self.task}_annotations")
    ...
    def get_mtat_top50_annotations(self):
        ...
        audio_path = "/import/research_c4dm/JulienMarcoChrisRMRI/MTAT_wav"
        ...
        annotations.loc[annotations["file_path"].str[:2].isin(val_folders), "split"] = (
            "val"
        )
        annotations.loc[
            annotations["file_path"].str[:2].isin(test_folders), "split"
        ] = "test"
    ...
    def get_mtat_all_annotations(self):
        ...
        annotations["split"] = "train"
        annotations.loc[annotations["mp3_path"].str[:2].isin(val_folders), "split"] = (
            "val"
        )
        annotations.loc[annotations["mp3_path"].str[:2].isin(test_folders), "split"] = (
            "test"
        )
        annotations = annotations.rename(columns={"mp3_path": "file_path"})
    ...
    def get_giantsteps_annotations(self):
        ...
        test_annotations["annotation_file"] = (
            ...
            + "/"
            + test_annotations["file_path"].str[:-4]
            + ".key"
        )
        test_annotations["file_path"] = (
            test_audio_path + "/" + test_annotations["file_path"]
        )
        ...
        train_annotations["file_path"] = (
            train_audio_path
            + "/"
            + train_annotations["file_path"].astype(str)
            + ".LOFI.mp3"
        )
        ...
        train_annotations["split"] = np.random.choice(
            ["train", "val"],
    ...
    def get_vocalset_singer_annotations(self):
        ...
        annotations = pd.DataFrame(annotations, columns=["singer_id", "file_path"])
        annotations["split"] = np.random.choice(
            ["train", "val", "test"],
            size=len(annotations),
            p=[1 - 2 * self.val_split, self.val_split, self.val_split],
        )
    ...
    def get_vocalset_technique_annotations(self):
        data_dir = "/import/c4dm-datasets/VocalSet1-2"
        train_singers = pd.read_csv(
            os.path.join(data_dir, "train_singers_technique.txt"), header=None
        )
        test_singers = pd.read_csv(
            os.path.join(data_dir, "test_singers_technique.txt"), header=None
        )
            ...
            for file in files:
                if file.endswith(".wav"):
                    ...
                    if singer_id in id_to_split.id.unique():
                        ...
                        split = id_to_split[id_to_split["id"] == singer_id][
                            "split"
                        ].values[0]

                        annotations.append([singer_id, file_path, technique, split])

        annotations = pd.DataFrame(
            annotations, columns=["singer_id", "file_path", "technique", "split"]
        )
        ...
        if self.val_split > 0:
            ...
            val_data = annotations[annotations["split"] == "train"].sample(
                frac=self.val_split
            )
    ...
    def get_mtg_annotations(self, path, audio_path):
        ...
        for split in ["train", "validation", "test"]:
            ...
            annotations.append(
                pd.DataFrame({"file_path": all_paths, "tags": all_tags, "split": split})
            )
    ...
    def get_medleydb_annotations(self):
        ...
        train, test = train_test_split(
            ...
            stratify=annotations["stem_instrument"],
        )
        ...
        idx2class = {
            i: c for i, c in enumerate(annotations["stem_instrument"].unique())
        }
        class2idx = {
            c: i for i, c in enumerate(annotations["stem_instrument"].unique())
        }
    ...
    def get_medleydb_raw_annotations(self):
        ...
        train, test = train_test_split(
            annotations,
            test_size=self.val_split,
            stratify=annotations["raw_instrument"],
        )
        train, val = train_test_split(
        ...
        idx2class = {i: c for i, c in enumerate(annotations["raw_instrument"].unique())}
        class2idx = {c: i for i, c in enumerate(annotations["raw_instrument"].unique())}
    ...
    def get_medleydb_both_annotations(self):
        ...
        for song in os.listdir(medleydb_path):
            ...
            for stem in yaml_dict["stems"]:
                ...
                row = {
                    "song_name": song.split("_METADATA.yaml")[0],
                    "filename": yaml_dict["stems"][stem]["filename"],
                    "filedir": yaml_dict["stem_dir"],
                    "stem_instrument": stem_instrument,
                }
        ...
        for song in os.listdir(medleydb_path):
            ...
            for stem in yaml_dict["stems"]:
                ...
                for raw in stem_dict["raw"]:
                    ...
                    raw_row = {
                        "song_name": song.split("_METADATA.yaml")[0],
                        "filename": stem_dict["raw"][raw]["filename"],
                        "filedir": yaml_dict["raw_dir"],
        ...
        df = df[
            df["stem_instrument"].isin(
                df["stem_instrument"].value_counts().head(20).index
            )
        ]
        raw_df = raw_df[
            raw_df["raw_instrument"].isin(
                raw_df["raw_instrument"].value_counts().head(40).index
            )
        ]

        df = pd.merge(
            df, paths2folders, left_on="song_name", right_on="path", how="inner"
        )
        df["file_path"] = df["folder"] + "/" + df["filedir"] + "/" + df["filename"]
        raw_df = pd.merge(
            raw_df, paths2folders, left_on="song_name", right_on="path", how="inner"
        )

        raw_df["file_path"] = (
            raw_df["folder"] + "/" + raw_df["filedir"] + "/" + raw_df["filename"]
        )
